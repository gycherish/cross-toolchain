target("gcc-env")
    ...
    on_build(function (target) 
        ...
        local package_info = package_env:get("toolchain.source.gcc")
        local major_version = package_info.version:match("^(%d+)")
        target:set("toolchain.gcc.source_dir", path.join(toolchain_env:get("toolchain.source_dir"), package_info.dirname))
        target:set("toolchain.native.gcc.build_dir", path.join(toolchain_env:get("toolchain.native.build_dir"), package_info.dirname))
        target:set("toolchain.native.gcc.cc", path.join(toolchain_env:get("toolchain.native.prefix"), "bin", "gcc"))
        target:set("toolchain.cross.gcc.build_bootstrap_dir", path.join(toolchain_env:get("toolchain.cross.build_dir"), package_info.dirname .. "-bootstrap"))
        target:set("toolchain.cross.gcc.build_final_dir", path.join(toolchain_env:get("toolchain.cross.build_dir"), package_info.dirname .. "-final"))
        ...
        target:set("toolchain.cross_native.gcc.build_dir", path.join(toolchain_env:get("toolchain.cross_native.build_dir"), package_info.dirname))
        ...
        target:set("toolchain.cross_native.gcc.cc1_dir", path.join(toolchain_env:get("toolchain.cross_native.prefix"), "libexec", "gcc", toolchain_env:get("toolchain.cross.target"), major_version))
        ...
        if option.get("verbose") then
            print("toolchain.source.gcc: ", package_info)
