target("isl-env")
    ...
    on_build(function (target) 
        ...
        local package_info = package_env:get("toolchain.source.isl")
        target:set("toolchain.isl.source_dir", path.join(toolchain_env:get("toolchain.source_dir"), package_info.dirname))
        target:set("toolchain.native.isl.build_dir", path.join(toolchain_env:get("toolchain.native.build_dir"), package_info.dirname))
        ...
        target:set("toolchain.cross.isl.build_dir", path.join(toolchain_env:get("toolchain.cross.build_dir"), package_info.dirname))
        ...
        if option.get("verbose") then
            print("toolchain.source.isl: ", package_info)
...
target("isl-download")
    ...
    on_build(function (target)
        ...
        local package_env = project.target("toolchain-package-env")
        package.download(package_env:get("toolchain.source.isl"), toolchain_env:get("toolchain.source_dir"))
        package.patch(package_env:get("toolchain.source.isl"), toolchain_env:get("toolchain.source_dir"))
        package.copy(function () 
            ...
            isl_env:get("toolchain.isl.source_dir")
        )
    end)
