target("musl-env")
    ...
    on_build(function (target) 
        ...
        target:set("toolchain.native.musl.build_dir", path.join(toolchain_env:get("toolchain.native.build_dir"), "musl-1.2.5"))
...
target("musl-cross-build")
    ...
    on_build(function (target)
        ...
        if os.exists(musl_env:get("toolchain.cross.musl.libc")) then
            print("musl libc has already built: ", musl_env:get("toolchain.cross.musl.libc"))
            return
        end
        local argv = {
